...
const double gw[] = { 0.555555555555556, 0.888888888888889, 0.555555555555556 };
// Mapped Gauss Points [0,1]
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    // Inputs: [dims, Xc, Yc, Zc, Ux, Uy, Uz, Uxn, Uyn, Uzn, params]
    ...
    for(int g=0; g<3; g++) {
        ...
        for(int n=0; n<3; n++) {
            Ph[n][g] = p[n]; DPh[n][g] = dp[n]; DDPh[n][g] = ddp[n];
        }
    }
        ...
        for (int ez = 0; ez < Nz; ez++) {
            for (int ey = 0; ey < Ny; ey++) {
                for (int ex = 0; ex < Nx; ex++) {
                    ...
                    // Gather Solution
                    double u_loc[27], v_loc[27], w_loc[27], un_loc[27];
                    double u_avg=0, v_avg=0, w_avg=0;
                    for(int i=0; i<27; i++) {
                        u_loc[i] = Ux[nodes[i]]; v_loc[i] = Uy[nodes[i]]; w_loc[i] = Uz[nodes[i]];
                        un_loc[i] = Ux_n[nodes[i]]; // Only needed for X component of history in scalar check?
                        // Actually need all components for history, but simplifying for brevity or assuming un_x is enough?
                        // Wait, code calculates b_supg for x,y,z. We need un, vn, wn.
                        // For SAFETY, let's grab them, but reusing un_loc for just X to save stack.
                        // NOTE: Correct logic needs un, vn, wn.
                            ...
                            for(int gx=0; gx<3; gx++) {
                                ...
                                for(int k=0; k<3; k++) {
                                    ...
                                    for(int j=0; j<3; j++) {
                                        ...
                                        for(int i=0; i<3; i++) {
                                            ...
                                            // Laplacian for SUPG Diffusion
                                            double ddx = ddpx*py*pz / (hx*hx);
                                            double ddy = px*ddpy*pz / (hy*hy);
                                            double ddz = px*py*ddpz / (hz*hz);
                                            Lap[idx] = ddx + ddy + ddz;
                                            ...
                                            un_val += Ux_n[nodes[idx]] * val;
                                            vn_val += Uy_n[nodes[idx]] * val;
                                            wn_val += Uz_n[nodes[idx]] * val;
                                ...
                                for(int r=0; r<27; r++) {
                                    ...
                                    for(int c=0; c<27; c++) {
                                        ...
                                        // 1. Advection + 2. SUPG Advection + 4. SUPG Time
                                        double val = rho * N[r] * u_grad_Nc + 
                                                     L_supg * (rho * u_grad_Nc) + 
                                                     L_supg * (rho/dt * N[c]);
                                        
                                        // 3. SUPG Diffusion (ADDED BACK)
                                        // Term: -mu * L_supg * Laplacian(Nc)
                                        val += (-mu) * L_supg * Lap[c];
